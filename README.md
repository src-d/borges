...
## Setting up borges configuration
...
* `CONFIG_TEMP_DIR`, by default: `/tmp/sourced`
* `CONFIG_BROKER`, by default: `amqp://localhost:5672`
* `CONFIG_ROOT_REPOSITORIES_DIR`,  (where to leave siva files) by default: `/tmp/root-repositories`
* `CONFIG_LOCKING`, by default: `local:`, other options: `etcd:`
* `CONFIG_HDFS`, (host:port) by default: `""`
...
To launch the producer you just have to run with the default configuration:

    borges producer

Producer reads repository's urls that it have to download from a database by default, but it can
read them directly from a file with the cli option:
...
You can select the number of worker the consumer use, by default it uses 8:
...
Borges has 2 runtime dependencies and has tests that depend on them:
  ...
  - PostgreSQL
    ...
    ```
    docker run --name postgres  -e POSTGRES_USER=testing -e POSTGRES_PASSWORD=testing  -p 5432:5432 -d postgres
